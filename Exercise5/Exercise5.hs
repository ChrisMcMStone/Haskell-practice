...
runMoves bs ((co, d):moves) | isPeg bs co && isPegNext bs co d && isValidPos bs co d = 
...
simulateMoves bs moves = case result of Nothing -> Nothing
                                        (Just xs) -> Just (countPeg xs)
        where
                result = runMoves bs moves
